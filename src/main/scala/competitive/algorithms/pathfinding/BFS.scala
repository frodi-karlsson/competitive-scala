...
class BFS[T] extends Pathfinder[T]:
    ...
    def findAllPaths(
        ...
        def recurse(curr: Node[T], dest: Node[T], path: Vector[Node[T]], result: Vector[Vector[Node[T]]]): Vector[Vector[Node[T]]] =
            if curr == dest && (!hasLength || path.length == length) then
                result :+ path
            else
                curr.visited = true
                ...
                newResult
        val res = recurse(start, end, Vector(start), Vector())
        graph.nodes.foreach(_.visited = false)
        res
...
object TestBFS:
    def testFindPath =
        ...
        val edges: Vector[Edge[Int]] = Vector(
